...
class PreferencesController extends GetxService {
  ...
  List<ResultModel> _loadResults() {
    try {
      ...
      return results!.map((result) => ResultModel.fromJson(jsonDecode(result))).toList();
    } catch (_) {
      return [];
    }
  }
  ...
  void saveResult(ResultModel result) {
    try {
      ...
      _repo.set<List<String>>(_resultsKey, _results.map((result) => jsonEncode(result.toJson())).toList());
    } catch (_) {}
  }
