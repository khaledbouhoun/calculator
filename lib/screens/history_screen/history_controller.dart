...
class HistoryController extends GetxController with StateMixin<List<ResultModel>> {
    ...
    switch (result) {
      case List<String> value:
        if (value.isEmpty) {
          ...
          final List<ResultModel> results = value.map((String e) => ResultModel.fromJson(jsonDecode(e))).toList();
  ...
  void addData(ResultModel resultModel) {
    final List<String> history = _database.get<List<String>>(AppConst.historyKey) ?? [];
