...
class SettingsController extends GetxController with StateMixin<SettingsModel> {
  ...
  void _loadSettings() {
    try {
      ...
      change(SettingsModel.fromMap(settingsMap), status: RxStatus.success());
    } catch (_) {
      change(SettingsModel.normal(), status: RxStatus.success());
    }
  }

  void updateSettings(SettingsModel settings) async {
    try {
      await _dataBase.set<String>(AppConst.settingsKey, jsonEncode(settings.toMap()));
      update();

      change(settings, status: RxStatus.success());
    } catch (e) {
      change(settings, status: RxStatus.error('Failed to save settings: $e'));
    }
  }
}
