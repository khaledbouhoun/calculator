...
import 'package:h_k_app/utils/result_model.dart';

var logger = Logger(printer: PrettyPrinter(), level: kReleaseMode ? Level.off : Level.debug);

extension CalculatorExtension on String {
  String get input => this;

  bool isNoSelection(int? offset) => offset == null || offset == -1 || offset == length;
  ...
  ResultModel calculate({bool skipErrorChecking = false, GrammarParser? parser}) {
    ...
    try {
      ...
      if (eval % 1 == 0 && !hasFraction) {
        return ResultModel(output: eval.toInt().toString(), expression: this, dateTime: DateTime.now());
      } else {
        return ResultModel(output: eval.toString(), expression: this, dateTime: DateTime.now());
      }
    } catch (e, stack) {
  ...
  bool get isNumber => double.tryParse(input) != null || int.tryParse(input) != null;
  ...
  (String, int) insertText(String value, int offset, {bool skipFormatting = false}) {
    try {
      ...
      } else if (lastPart.startsWithAny(['x', '÷', '+', '-', '%']) && !value.isNumber) {
  ...
  (String, int) removeCharAt(int offset) {
    try {
      ...
      logger.e('Error removing char at offset $offset', stackTrace: stack, error: e);
      return (input, -1);
    }
  }
  ...
  String get _formatDouble {
    ...
    if (number.endsWith('.') && number.substring(0, number.length - 1).contains('.')) {
      shouldAddDecimal = false;
      number = number.substring(0, number.length - 1);
    }
    final numOfDecimalPlaces = number.split('.').last == '0' ? 1 : number.split('.').last.length;
    NumberFormat formatter = NumberFormat.decimalPatternDigits(decimalDigits: numOfDecimalPlaces);
    return formatter.format(double.tryParse(number)) + (shouldAddDecimal ? '.' : '');
  }
}
